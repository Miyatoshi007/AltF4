...
<body>
	...
	<div class="main-content">
		<h2 class="text-start mb-5">Danh sách khóa học đã mua</h2>

		<!-- Check if listEnl is empty -->
		<c:if test="${empty listEnl}">
			<p class="text-center text-muted">Bạn chưa mua khóa học nào.</p>
		</c:if>

		<!-- Course List -->
		<div class="row row-cols-1 row-cols-md-3 row-cols-lg-5 g-4">
			<c:forEach var="enrollment" items="${listEnl}">
				<div class="col">
					<div class="card course-card">
						<img
							src="${empty enrollment.course.image ? 'https://via.placeholder.com/300x180' : enrollment.course.image}"
							class="card-img-top" alt="${enrollment.course.name}" />
						...
						<div class="card-footer">
							<button class="btn btn-custom"
								onclick="window.location.href='/ASM/user/course/courseLesson?courseId=${enrollment.course.id}'">
								Xem khóa học</button>
						</div>
					</div>
				</div>
